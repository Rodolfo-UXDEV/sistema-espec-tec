...
export default function ImageUploader({ image, setImage, readOnly = false }) {
  ...
  return (
    <div className="w-full">
      {!image ? (
        readOnly ? (
          <div className="flex min-h-[150px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-slate-400 dark:border-slate-800 dark:bg-slate-900/50">
            <svg className="h-8 w-8 text-slate-350 dark:text-slate-600 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <span className="mt-2 text-xs font-semibold text-slate-400 dark:text-slate-500">Sem mockup disponível</span>
          </div>
        ) : (
          <div
          ...
          >
            ...
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-400 shadow-sm transition-transform duration-300 group-hover:scale-110 dark:bg-slate-800">
              <svg
                className="h-6 w-6 text-slate-550 dark:text-slate-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
